...
// 邻接矩阵类型的方法声明:
	...
	int hasCycleViolent();
	void SecShortestPath(ElemType& a, ElemType& b);  //求次短路径
	void dfs(int u, int v, vector<int> path, int flag);  //遍历两顶点的所有路径
	void superpath(ElemType& a, ElemType& b);   //万能搜索路径
...
template<class ElemType>
void AdjMatrixUndirGraph<ElemType>::dfs(int u, int v, vector<int> path, int flag) {  //找出所有从u到v的路径存到全局变量allPath中
...
template<class ElemType>
void AdjMatrixUndirGraph<ElemType>::SecShortestPath(ElemType& a, ElemType& b) {
		
	int u, v;  vector<int> path;//起点终点对应的序号
	int i = 0, j = 0;
	for (i = 0; i < vexNum; i++) {
		if (vertexes[i] == a) break;
	}
	if (i == vexNum) {cout << "无该顶点" << endl; return;}
	else { u = i; }
	for (i = 0; i < vexNum; i++) {
		if (vertexes[i] == b) break;
	}
	if (i == vexNum) { cout << "无该顶点" << endl; return; }
	else { v = i; }                                    //找到a,b对应信息的序号
	...
	if (allPath.size() < 2) { cout << "无次短路径"; return; }  //判断有无次短路径
		...
		sort(L, L + length);
		for (j = 0; j < length; j++) {                     //找出次短路径
			if (L[j] == h[1]) break;
		}
		//打出路径
		cout << "次短路径值为" << h[1] << endl;
		cout << "路径为:";
		cout << vertexes[u] << "->";
...
template<class ElemType>
void AdjMatrixUndirGraph<ElemType>::superpath(ElemType& a, ElemType& b)
{
	int u, v;  vector<int> path;//起点终点对应的序号
	int i = 0, j = 0;
	for (i = 0; i < vexNum; i++) {
	...
	dfs(u, v, path,0); 
	if (allPath.size() == 0) { cout << "无任何路径"; return; }  //判断有无次短路径
	else {
		int length = allPath.size();   //得到路径总数
		int* L = new int[length];  //每条路径权重值
		int* h = new int[length];  //复制一个L
		...
		for (i = 0; i < length; i++) h[i] = L[i];
		sort(h, h + length);
		int* t = new int[length]; //储存h对应的路径的位置
		for (i = 0; i < length; i++) {
			for (j = 0; j < length; j++)
			{
				if (h[i] == L[j]) {
		...
		//打出路径
		cout << "起点到终点的路径一共有" << length << "条" << endl;
		cout << "路径分别为:" << endl;
		for (j = 0; j < length; j++) {
			cout << j + 1 << ":" << vertexes[u] << "->";
			for (i = 0; i < allPath[t[j]].size()-1; i++)
		cout << vertexes[allPath[t[j]][i]] << "->";
			
			cout << vertexes[v] << endl;
			cout << "\t" << "经过顶点数为:" << allPath[t[j]].size()+1 << "\t" << "路径长度为:" << h[j] << endl;
		} 
	}
